...
async function waitReady(inst,timeout=180000){const start=Date.now();while(Date.now()-start<timeout){if(inst.proc?.exitCode!=null)throw new Error(`llama-server kapandı. ${inst.logs.slice(-10).join('\n')}`);try{const r=await fetch(`http://127.0.0.1:${inst.port}/health`);if(r.ok)return;}catch{}await sleep(350);}throw new Error('llama-server zaman aşımına uğradı.');}
...
export async function loadModel(id,options={}){
  ...
  const model=await getInstalled(id);if(!model)throw new Error('Model kurulu değil.');if(model.runtime!=='llama.cpp')throw new Error('Bu model llama.cpp ile çalışmıyor.');
  ...
  const exe=await ensureLlamaCpp();const port=nextPort++;const inst={id,port,proc:null,logs:[],options:o,jit:o.jit,ttl:o.ttl,lastUsed:Date.now(),busy:false,startedAt:Date.now(),model};
  const args=argsFor(model,port,o);inst.proc=spawn(exe,args,{windowsHide:true,stdio:['ignore','pipe','pipe']});inst.proc.stdout.on('data',d=>pushLog(inst,d));inst.proc.stderr.on('data',d=>pushLog(inst,d));inst.proc.on('exit',()=>{instances.delete(id);});instances.set(id,inst);
